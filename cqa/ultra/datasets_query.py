...
class LogicalQueryDataset(InMemoryDataset):
    ...
    struct2type = {
        ...
        ("e", ("r", "r")): "2p",
        ("e", ("r", "r", "r")): "3p",
        ("e", ("r", "r", "r", "r")): "4p",                                   # is-cqa +H (eval-only)
        (("e", ("r",)), ("e", ("r",))): "2i",
        (("e", ("r",)), ("e", ("r",)), ("e", ("r",))): "3i",
        (("e", ("r",)), ("e", ("r",)), ("e", ("r",)), ("e", ("r",))): "4i",  # is-cqa +H (eval-only)
        ((("e", ("r",)), ("e", ("r",))), ("r",)): "ip",
        (("e", ("r", "r")), ("e", ("r",))): "pi",
        (("e", ("r",)), ("e", ("r", "n"))): "2in",
        (("e", ("r",)), ("e", ("r",)), ("e", ("r", "n"))): "3in",
...
class InductiveFB15k237Query(LogicalQueryDataset):
    ...
    def load_queries(self, path):
        ...
        for split in ["train", "valid", "test"]:
            if split == "train" and not os.path.exists(os.path.join(path, "train_queries.pkl")):
                # evaluation-only copy of a benchmark (valid/test queries only)
...
# --------------------------------------------------------------------------- #
# Complexity-aware "+H" benchmarks (is-cqa-complex strategy). The inductive     #
# and fully-inductive sets are a release asset unpacked to cqa/benchmarks/     #
# (see the README, Data) and can be regenerated with                          #
# cqa/script/create_queries.py; the code never downloads them.                #
# Per-(type, tier) reduction folders ({raw_dir}/test-query-reduction/) are      #
# built by cqa/script/build_reductions.py and read by run_query.py --test.      #
# --------------------------------------------------------------------------- #
def _no_remote_download(self):
    raise FileNotFoundError(
        f"{self.raw_dir}: expected files {self.raw_file_names} are missing. "
        f"See the README (Data) for where each +H benchmark comes from."
    )


class InductiveFB15k237QueryH(InductiveFB15k237Query):
    """+H variant of InductiveFB15k237Query (entity-inductive), evaluation only.
    --version 550 reads root/550H/."""

    @property
    def name(self):
        return f"{self.version}H"

    @property
    def raw_file_names(self):
        return ["valid_queries.pkl", "test_queries.pkl"]

    download = _no_remote_download

    def __repr__(self):
        return f"fb_{self.version}H"


class WikiTopicsQueryH(WikiTopicsQuery):
    """+H variant of WikiTopicsQuery (fully inductive: new entities and relations),
    evaluation only. --version tax reads root/WikiTopics_QE/taxH/."""
    ...
    def __repr__(self):
        return f"wikitopics_{self.version}H"


# ---------------------------------------------------------------------------
# Official is-cqa-complex +H benchmarks (github.com/april-tools/is-cqa-complex,
# release `benchs-1.0`, asset iscqa-compl-benchmarks.zip -> new_benchmarks/).
# These are the transductive +H sets.
#
# Notes:
#   * all three parse with struct2type: 16 types incl. 4p / 4i, DNF unions;
#   * inverse relations follow the BetaE convention (inv = r XOR 1), which is what
#     LogicalQueryDataset.process() assumes (inverse_rel_plus_one=True);
#   * ICEWS18+H ships its splits under KG_splits/(train|valid|test).txt; link them
#     into the dataset dir (see the README).
# Point dataset.root at .../iscqa-compl-benchmarks/new_benchmarks/.
# ---------------------------------------------------------------------------
class FB15k237IsCqaH(LogicalQueryDataset):
    """Official is-cqa-complex FB15k-237+H (transductive)."""
...
class NELL995IsCqaH(LogicalQueryDataset):
    """Official is-cqa-complex NELL995+H (transductive)."""
...
class ICEWS18IsCqaH(LogicalQueryDataset):
    """Official is-cqa-complex ICEWS18+H (transductive).

    Splits come from KG_splits/
    via symlinks (see note above)."""
