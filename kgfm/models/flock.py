...
# Optional dependency: only Flock needs the vendored graph-walker extension,
# but kgfm.models imports this module eagerly — don't break ULTRA/MOTIF/TRIX
# environments that never built it.
try:
    import graph_walker
except ImportError:
    graph_walker = None
...
from kgfm.models.consensus import consensus_mean, consensus_softmax_stable
...
class Flock(nn.Module):
    def __init__(self, model_cfg: dict):
        ...
        if graph_walker is None:
            raise ImportError(
                "Flock requires the vendored graph-walker extension: "
                "pip install -e ./graph-walker --no-build-isolation "
                "(see README, Installation)"
            )
        # Upstream flock sets this at module level (src_entity/flock/models.py:13),
        # but kgfm.models imports every model module eagerly, so a module-level
        # call would flip TF32 on for Ultra/MOTIF/TRIX runs too. Setting it here
        # keeps upstream Flock numerics (it always ran with TF32 active) without
        # affecting the other models.
    ...
    def forward(self, data: Data, batch: Tensor, precomputed_rel_emb=None):
        """
        Arguments:
            data:
                edge_index: [2, |E|]
            ...
            precomputed_rel_emb: unused; accepted because the shared eval loop
                (script/run.py test/test_visibility) passes it to every model.
        Return:
            score: [batch_size, num_negative + 1]
        """
        device = data.device
