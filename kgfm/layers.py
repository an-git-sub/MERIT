...
class HypergraphLayer(nn.Module):
    ...
    def forward_sharded(self, node_features, query, hypergraph_shards):
        ...
        out = self._triton_kernel_apply(node_features_flatten, rowptr, indices, node_size,
                                        etypes, relation_vector, pos_embedding, pos_index)
        # Free arity-2 GPU intermediates before the arity-3 chunks need the headroom.
        # preprocess_triton_hypergraph is recomputed on each forward, so there's no
        # value in keeping these alive past the kernel call.
        ...
        # variable holds the old chunk while the new one is being allocated,
        # doubling peak GPU memory.
