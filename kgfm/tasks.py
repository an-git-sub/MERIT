...
def build_relation_graph(graph):
    ...
    # at training time (ultra.py rebuilds the relation_graph per batch
...
# to avoid shadowing ULTRA's `build_relation_graph` above (which produces a
...
def build_relation_adj(graph):
    ...
    Lifted verbatim from TRIX's `build_relation_graph`; renamed to keep ULTRA's
    `build_relation_graph` (which produces `graph.relation_graph` instead) intact.

    Runs in two contexts:
    - Pre-collate, as part of `kgfm/datasets.py`'s `pre_transform` chain, which
      caches `relation_adj` in `processed/data_relation_graph_entity.pt`.
    - Post-collate, via `script/run.py:ensure_relation_adj` (and equivalents in
      run_relation/run_many*/pretrain*), a runtime fallback. No-op when cached.
    ...
    # the dataset is loaded, so we cast back to plain ints, which PyG
    # MessagePassing's shape inference inside RelNet expects.
...
def build_motif_tensor(graph):
    """Dense relational motif tensor `M` of shape [R, R, 4] used by MERIT.

    `M[r, p, t] = log1p(n)`, where n is the number of entities shared by relations r
    and p in motif channel t: head-head, tail-tail, head-tail, tail-head (the four
    edge types of ULTRA's relation graph, with the same sparsity pattern). Runs on the
    graph's device, so it can be rebuilt inside the training forward.
    """
    edge_index, edge_type = graph.edge_index, graph.edge_type
    num_nodes, R = graph.num_nodes, int(graph.num_relations)
    device = edge_index.device
    edge_type = edge_type.to(device)

    # (node, relation) incidences, E[node, rel] = 1, and their transposes
    Eh_idx = torch.vstack([edge_index[0], edge_type]).T.unique(dim=0)
    Et_idx = torch.vstack([edge_index[1], edge_type]).T.unique(dim=0)
    Eh = torch.sparse_coo_tensor(Eh_idx.T, torch.ones(Eh_idx.shape[0], device=device), (num_nodes, R))
    Et = torch.sparse_coo_tensor(Et_idx.T, torch.ones(Et_idx.shape[0], device=device), (num_nodes, R))
    EhT = torch.sparse_coo_tensor(torch.flip(Eh_idx, dims=[1]).T, torch.ones(Eh_idx.shape[0], device=device), (R, num_nodes))
    EtT = torch.sparse_coo_tensor(torch.flip(Et_idx, dims=[1]).T, torch.ones(Et_idx.shape[0], device=device), (R, num_nodes))
    ...
    M = torch.zeros(R, R, 4, device=device)
    for t, A in enumerate((Ahh, Att, Aht, Ath)):
        idx = A.indices()
        M[idx[0], idx[1], t] = torch.log1p(A.values())
    return M
