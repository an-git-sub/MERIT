...
from kgfm.tasks import build_relation_graph, build_relation_hypergraph, build_relation_adj, build_motif_tensor


def build_motif(graph):
    """MERIT's eval-time cache: the motif tensor M ([R, R, 4]) on `graph.motif`.
    Wrapped in a `Data` so PyG keeps it per split (R can differ across the splits of
    an inductive dataset) instead of concatenating it."""
    M = build_motif_tensor(graph)
    graph.motif = Data(num_nodes=int(M.shape[0]), M=M)
    return graph
...
# both TRIXEntity and TRIXRelation read the same file. "motif" holds MERIT's
# motif tensor for evaluation.
BUILDERS = {
