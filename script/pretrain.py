...
def train_and_validate(cfg, model, train_data, valid_data, filtered_data=None, batch_per_epoch=None):
    ...
    if world_size > 1:
        # TRIX-family models call entity_model_mini inside relation_model's mid-pass
        # but only consume its `feature` output, leaving entity_model_mini.mlp params
        ...
        find_unused = cfg.model["class"] == "TRIXEntity"
    ...
    for i in range(0, cfg.train.num_epoch, step):
        ...
        # pretraining keeps the last epoch
        best_epoch = epoch
...
if __name__ == "__main__":
    ...
    elif cfg.model["class"] == "MERIT":
        model = MERIT(
            rel_model_cfg=cfg.model.relation_model,
            entity_model_cfg=cfg.model.entity_model,
        )
    elif cfg.model["class"] == "Flock":
